...
#         # =================================================
#         # ★ 분석 결과 반영: dim_feedforward=1024, backbone="resnet18"
#         # =================================================
...
#             dim_feedforward=1024,   # <--- 분석결과 반영
#             backbone="resnet18"     # <--- 분석결과 반영
#         ).to(device)
...
#     if size == 4:
#         handler.write4ByteTxRx(bus.port_handler, motor.id, addr, raw_int)
#     else:
#         handler.write2ByteTxRx(bus.port_handler, motor.id, addr, raw_int)
...
#                 target_raw_list.append(int(raw_val))
#                 manual_raw_write(bus, name, raw_val)

#             print(f"\r[Raw] Tgt: {target_raw_list}", end="")
...
def manual_raw_write(bus, motor_name, raw_value):
    ...
    if size == 4:
        ...
        # ★ 비트 연산: 파이썬의 음수를 32비트 Unsigned int(2의 보수)로 변환
        # 예: -50000 -> 4294917296
...
def main():
    ...
    try:
        while True:
            ...
            target_raw_list = []
            for i, name in enumerate(JOINT_NAMES):
                pred_val = float(next_action[i]) # 모델 예측값 (-50 ~ 50 예상)

                # ★ 기어비 보정: 
                # 모델은 -50~50으로 학습했으므로, 11번 모터(Follower)의 경우 
                # 2배를 곱해줘야 원래 의도한 -100~100 범위가 됩니다.
                if name in bus.motors:
                    motor_id = bus.motors[name].id
                    if motor_id in GEAR_RATIOS:
                        pred_val = pred_val * GEAR_RATIOS[motor_id]

                # 1. denormalize: -100~100 값을 실제 모터 raw 값(음수 가능)으로 변환
                raw_val = bus.denormalize(name, pred_val)
                target_raw_list.append(int(raw_val))
                
                # 2. write: 비트 연산이 포함된 쓰기 함수 호출
                manual_raw_write(bus, name, raw_val)
